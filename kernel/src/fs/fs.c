...
static const file_info file_table[] = {
	...
	{"credits_bgm.wav", 6631340, 1048576},
};
...
size_t fs_read(int fd, void *buf, size_t len)
{
	...
	if(files[fd].offset + len >= file_table[fd - 3].size)
		nlen = file_table[fd - 3].size - 1 - files[fd].offset;
	ide_read(buf, file_table[fd - 3].disk_offset + files[fd].offset, nlen);
	files[fd].offset += nlen;
	
	if(len > nlen) memset(buf + nlen, 0, len - nlen);
	return nlen;
	//*/return -1;
}
